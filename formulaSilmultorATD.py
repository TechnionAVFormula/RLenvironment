...
def transform(s):
   ...
   # plt.show()
    ...
    bottom_black_bar_bw = cv2.resize(
        bottom_black_bar_bw, (840, 120), interpolation=cv2.INTER_NEAREST)
    img2 = cv2.cvtColor(upper_color_bar, cv2.COLOR_RGB2GRAY)
    upper_color_bar_bw = cv2.threshold(img2, 1, 255, cv2.THRESH_BINARY)[1]
    upper_color_bar_bw = cv2.resize(
        upper_color_bar_bw, (840, 840), interpolation=cv2.INTER_NEAREST)
    ...
    upper_field_bw = cv2.resize(
        upper_field_bw, (10, 10), interpolation=cv2.INTER_NEAREST)  # re scaled to 7x7 pixels
...
#     print(car_field_bw[:, 3].mean()/255, car_field_bw[:, 4].mean()/255, car_field_bw[:, 5].mean()/255, car_field_bw[:, 6].mean()/255)
    car_field_t = [car_field_bw[:, 3].mean()/255, car_field_bw[:, 4].mean() /
                   255, car_field_bw[:, 5].mean()/255, car_field_bw[:, 6].mean()/255]
...
def compute_steering_speed_gyro_abs(a, count):
    ...
    # (right_steering - left_steering + 1.0)/2
    steering = (right_steering - left_steering)/2.0
...
class SilumationDetails:
    ...
    def DoNextAction(self, action):
        ...
        self.observation, self.reward, self.done, self.information = self.environment.step(
            action)
        bottom_black_bar_bw, upper_field_bw, car_field_t, upper_black_bar_bw = transform(
            self.observation)
        self.count += 1
        details = []
        details = compute_steering_speed_gyro_abs(
            bottom_black_bar_bw, self.count)
        self.actualSteering = details[0]
        self.actualSpeed = details[1]
        self.actualGyro = details[2]
    ...
    def PlotSpeed(self):
        speed = np.array(self.speedDiagram, dtype=None,
    ...
    def PlotGyroDiagram(self):
        gyro = np.array(self.gyroDiagram, dtype=None, copy=True,
    ...
    def PlotSteering(self):
        steering = np.array(self.steeringDiagram, dtype=None,
                            copy=True, order='K', subok=False, ndmin=0)
...
if __name__ == "__main__":
    mySilmulator = SilumationDetails(num_of_episodes = 100,
                                     num_of_steps = 100)
